...
3. ReleaseGuard 继续作为独立主仓库。默认直接使用经 G0 验证并锁定的上游发布镜像与清单，由本仓库的 Kustomize overlay 管理环境差异；不复制完整上游源码，不使用 Git submodule，也不以 Online Boutique fork 替代本仓库。
4. 只有在 overlay、exporter、采集适配或合成检查不能满足已验证的遥测缺口，且确实需要修改上游业务源码时，才建立独立 fork。该 fork 仅作为运行依赖，必须记录 base commit、最小补丁、构建方法与上游同步策略。
5. 首版覆盖无新发布的购物车至 Redis 故障链路，部署必要业务依赖，但不承诺所有服务的诊断能力。
6. Kubernetes 最小运行底座提前，首选本地 Kind；GitOps、Argo Rollouts、云平台和完整 traces 延后。
7. v0.1 交付真实告警、只读 Agent、人工恢复和独立验证；v0.2 加强调查可靠性；v0.3 才开放一种经审批的真实写动作；v1.0 增加平台深化与发布专项。
8. 发布上下文作为可选证据，不要求所有事故存在 baseline/candidate。保留旧发布接口与测试，监控契约以新增版本迁移。
9. 每阶段同步交付报告、失败结果、演示和双方贡献证据；首版不宣称自动修复或生产可用。
...
| 用 Online Boutique fork 替代 ReleaseGuard 仓库 | 业务代码与部署素材集中 | 淹没原创系统及提交历史，继承无关 CI 与维护面，并混淆上游和本项目贡献 |
...
- ReleaseGuard 的仓库身份、Issue/PR 历史和原创制品保持独立；上游身份通过锁定清单进入部署、报告和评测记录。
- 平台开发量转向上游版本锁定、数据适配、告警质量、故障清理和可靠运行；不会因为复用应用而自动获得完整可观测性。
- 默认集成路径减少上游代码维护，但要求从干净 clone 验证下载、校验和 overlay 渲染；上游升级必须显式执行，不能跟随浮动 `main/latest`。
- 首版降低自动执行范围，保留人工操作审计与独立验证。自动处置是后续明确 Gate，不能在作品集中提前声明。
- 项目版本与 API 版本分别管理；本次仅调整计划文档，不静默修改现有运行契约。

## 实施前验证与复审条件

G0 必须验证实际上游资源、Redis 场景、业务信号、日志来源和清理行为，锁定具体 release、commit、镜像 digest、清单来源与许可证，并证明仅凭 ReleaseGuard 仓库中的锁定清单和 overlay 可以从干净 clone 重建环境。若资源需求不可接受、故障无法稳定复现或关键证据缺失，记录实验并调整部署/采集方案，不能用想象中的能力通过验收。

G0 还必须给出 `fork_required` 判断。默认值为 `false`；只有记录了无法通过 overlay、exporter、采集适配或合成检查弥补的源码级缺口，才可以改为 `true` 并创建独立 fork。创建 fork 或首次扩大其中的补丁范围属于需要复审的架构变化。
