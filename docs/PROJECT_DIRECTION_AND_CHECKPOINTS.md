...
被监控的业务应用以外部、版本锁定的运行依赖方式集成 Google Online Boutique。ReleaseGuard 保持为独立主仓库，默认消费经验证的上游发布镜像与清单，并在本仓库维护 Kustomize overlay；不复制完整上游源码，也不以 Online Boutique fork 替代本仓库。业务代码由上游提供；我们的成果是集成、监控与告警工程、调查 Agent、证据契约、恢复验证和可复现评测。保留 ReleaseGuard 名称，发布回归作为后续专项能力，部署与 Git 变更属于可选上下文。
...
| Google Online Boutique 上游 | 不并入主仓库；G0 锁定发布制品和许可证，本仓库只维护来源、校验、overlay 与必要的最小集成补丁 |
...
- [ ] 在 `platform/online-boutique/` 提交机器可读的上游锁定清单，至少包含仓库 URL、release、commit、清单来源/校验、实际镜像 digest、许可证/NOTICE 状态和 `fork_required` 判断。
- [ ] 从 ReleaseGuard 干净 clone 仅使用锁定清单、可复现获取流程和 Kustomize overlay 完成部署与渲染检查；不依赖本地手工修改的上游 YAML。
...
- [ ] 检查实际上游埋点与字段；优先增加最小合成检查、exporter 或采集适配，不重新开发业务服务。只有这些方式无法弥补已记录的源码级缺口时，才创建独立 fork，并记录 base commit、最小补丁、构建方法及同步策略。
- [ ] 故障注入具备作用域校验、原状态保存、独立 TTL 清理和幂等恢复。

退出证据：健康/异常采样、遥测覆盖矩阵、上游锁定清单、干净 clone 重建记录、`fork_required` 决策、资源记录和故障恢复记录。若业务症状或资源条件不成立，先明确记录并修正载体，不带着未验证假设进入 G1。
...
| 为什么使用现成应用且不直接 fork？ | 将精力投入监控、调查与平台，保持原创仓库边界；注明上游贡献 | 锁定版本、overlay、fork 决策、必要集成补丁及双方原创制品 |
...
- 上游制品必须可追溯到锁定清单；实际运行身份不匹配时不能计入 live 验收。上游业务源码若需修改，必须在独立 fork 中保留最小补丁与 base commit，不并入 ReleaseGuard 主仓库。
...
| 顺序 | 工作 | Owner | 完成依据 |
|---:|---|---|---|
| 1 | 验证并锁定 Online Boutique 与 Redis 场景，完成 fork 必要性判断 | 平台牵头，双方取证 | G0 锁定清单、干净 clone 重建、覆盖矩阵、资源与清理记录 |
